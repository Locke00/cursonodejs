...
selector.addEventListener("click", async () => {  //cada click en el boton va a generar ejecutar el cb
  ...
  try {
    ...
    location.replace("/sessions/login")   //aqui se deberia redirigir a login si se registro con exito, pero hay 
                             // q completar para q funcione bien, xq la linea de arriba no hace esa verificacion
  } catch (error) {
    alert(error.message);
  }
});
