...
// ademas, passport se encarga de la creacion del usuario, xq lo q ya no tengo q poner eso en el cuerpo de este endpoint
...
sessionsRouter.post("/login", passport.authenticate("login", {session: false, failureRedirect: "/api/sessions/badauth"}), async (req, res, next) => {
  try {
    return res.json({
      statusCode: 200,
      message: "Logged in",
      session: req.session,  // aqui tb defino la sesion
    });
  } catch (error) {
    return next(error);
  }
