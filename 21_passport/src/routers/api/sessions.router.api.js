  ...
  async (req, res, next) => {
    try {
      return res.json({
        ...
        token: req.token,   //le devuelvo el token para q lo guarde, y lo reenvie al server en cada peticion
      });
    } catch (error) {
      return next(error);
    }
...
//google      , cuando vaya a este endpoint me va a llevar a la pantalla de consentimiento de OAuth
...
//DEBE SER UN POST PARA LO Q EJECUTE EL EVENTO DE CLICK EN EL BOTON DE GOOGLE. ese boton tiene q fetchear en este en point
sessionsRouter.get(
  "/google",
