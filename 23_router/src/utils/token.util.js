...
function createToken(data) {
  ...
  const token = jwt.sign(
    ...
    process.env.SECRET,
    { expiresIn: 5 }//60 * 60 * 24 * 7 } //fecha de caducidad de la contraseñas (es en segs)  (puse 7 dias)
  );
...
function verifytoken(token) {
  ...
  //const token = headers.cookies; //tengo q evaluar si existe y si es correcto
  if (token) {
