...
sessionsRouter.post(
  ...
  async (req, res, next) => {
    try {       // httpOnly: true   permite q no se pueda acceder a la cookie dsd el navegador
      return res.cookie("token", req.token, {maxAge: 7*24*60*60, httpOnly: true}).json({
        statusCode: 200,
        message: "Logged in",
...
sessionsRouter.post("/signout", passport.authenticate("jwt",{ session:false, failureRedirect: "/api/sessions/signout/cb" }), async (req, res, next) => {
  try {
